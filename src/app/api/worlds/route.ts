...
export async function POST(request: NextRequest) {
    try {
        ...
        if (existingWorld) {
            // If the owner no longer exists (orphaned world from failed account deletion),
            // auto-delete the stale world and allow recreation
            if (!existingWorld.owner) {
                console.log(`[CREATE_WORLD] Found orphaned world "${existingWorld.name}" with no owner, deleting...`)
                await prisma.world.delete({ where: { id: existingWorld.id } })
                // Fall through to create the new world
            } else if (existingWorld.ownerId === userId) {
                return NextResponse.json(
                    { error: 'You already have a world for this Discord server.' },
                    { status: 409 }
                )
            } else {
                return NextResponse.json(
                    { error: `A world already exists for this Discord server (owned by ${existingWorld.owner.name || 'another user'}).` },
                    { status: 409 }
                )
            }
        }
        ...
        return NextResponse.json(
            {
                error: `Failed to create world: ${details}`,
            },
            { status: 500 }
        )
    }
