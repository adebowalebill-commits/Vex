...
export async function GET(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        // session.user.id might be DB ID or Discord ID, handle both
        const sessionId = session.user.id
        let hasAccess = world.ownerId === sessionId
        if (!hasAccess) {
            // Check if session user's Discord ID maps to the owner
            const discordId = (session.user as any).discordId || sessionId
            const ownerUser = await prisma.user.findUnique({
                where: { id: world.ownerId },
                select: { discordId: true }
            })
            if (ownerUser?.discordId === discordId) hasAccess = true
        }
        if (!hasAccess) {
            // Also check citizen access
            const citizenAccess = await prisma.citizen.findFirst({
                where: { worldId, userId: sessionId },
            })
            if (!citizenAccess) {
                // Try by Discord ID
                const discordId = (session.user as any).discordId || sessionId
                const userByDiscord = await prisma.user.findUnique({ where: { discordId } })
                if (userByDiscord) {
                    const citizenByDbId = await prisma.citizen.findFirst({
                        where: { worldId, userId: userByDiscord.id },
                    })
                    if (citizenByDbId) hasAccess = true
                }
            } else {
                hasAccess = true
            }
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        // Check ownership — handle Discord ID vs DB ID
        let isOwner = world.ownerId === session.user.id
        if (!isOwner) {
            ...
            if (ownerUser?.discordId === discordId) isOwner = true
        }
